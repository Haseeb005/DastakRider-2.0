...
- **`products[].count` is the ordered count; `products[].quantity` is NOT a number — it's the size/variation descriptor** ("Half", "Regular", "Full", "12 Pcs", or even a full combo string). There is no `size`/`variation` field, so the displayed size must fall back to `quantity` (for non-deal items). Never use `quantity` as the numeric multiplier — that's `count`.
...
**`updatedAt` is bulk-written and unreliable for time filtering — use `createdAt`.** Every `Delivered` order in the shared prod DB carries one identical `updatedAt` (a bulk admin/migration write), so any Today/Week/Month filter on `updatedAt` returns the overall total. Period filtering (history list + earnings today/week/month buckets) must use `createdAt`, which has a real distribution and aligns with the PKT period helper. There is no full delivery datetime (`deliveredAt` doesn't exist; only `timeWhenDelivered` time-string, `date`/`dateForSearching` date-only); `createdAt` is the best full-precision proxy since orders are same-day delivery.
